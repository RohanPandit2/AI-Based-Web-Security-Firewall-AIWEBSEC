...
def extract_features(method, path_enc, body_enc, headers):
    ...
    # Fixing the encoding issue for body
    body_bytes = body.encode('utf-8') if isinstance(body, str) else body

    return [method, path_enc.encode('utf-8').strip(), body_bytes.strip(), single_q, double_q, dashes,
            braces, spaces, badwords_count, "1"]


resultt = parse_log(log_path)

with open('httplog.csv', "w", newline='', encoding='utf-8') as file:
    c = csv.writer(file)
